...
export class VAxios {
    ...
    private setupInterceptors() {
        ...
        const { requestInterceptors, responseInterceptorsCatch } = this.getTransform() || {}
    ...
    request<T = any>(config: AxiosRequestConfig, options?: RequestOptions): Promise<Result<T>> {
        const { beforeRequestHook, transformRequestHook } = this.getTransform() || {}
        ...
        return new Promise((resolve, reject) => {
            this.axiosInstance.request(conf).then(async (res: AxiosResponse<Result<T>>) => {
