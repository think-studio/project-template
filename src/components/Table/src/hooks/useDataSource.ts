...
export function useDataSource(
  ...
  function handlePagintionChange(pagination: {
    pageSize?: number;
    current?: number;
  ...
  async function fetch(opt?: FetchParams) {
    let timer;
    const { api, searchInfo, pagination = true, headerConfig = {} } = unref(propsRef);
    ...
    try {
      ...
      let params = merge(searchInfo, useSearchForm ? getFieldsValue() : {}, pageParams, opt?.searchInfo ?? {});
