<template>
    <el-popover :placement="placement" trigger="focus" :hide-after="0" :width="state.selectorWidth"
                ...
                <div class="icon-selector-box">
                    <div class="selector-header">
                        ...
                        <div class="selector-tab">
                            <span @click="onChangeTab('ele')"
                                :class="state.iconType == 'ele' ? 'active' : ''">ele</span>
                            <span @click="onChangeTab('awe')"
                                :class="state.iconType == 'awe' ? 'active' : ''">awe</span>

                            <span @click="onChangeTab('local')"
        ...
        <template #reference>
            <el-input v-model="state.inputValue" :size="size" :disabled="disabled" placeholder="请选择图标"
                ref="selectorInput" @focus="onInputFocus" @blur="onInputBlur" :class="'size-' + size">
                <template #prepend>
                    <div class="icon-prepend">
                        <Icon :key="'icon' + state.iconKey"
                            :name="state.prependIcon ? state.prependIcon : state.defaultModelValue" />
                        <div class="name">{{ state.prependIcon ? state.prependIcon :
