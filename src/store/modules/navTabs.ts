...
export const useNavTabStroe = defineStore('navTab', {
    ...
    actions: {
        ...
        setActiveRoute(route: RouteLocationNormalized | BackMenuModel) {
            ...
            const currentMenuIndex = toRaw(this.tabsView).findIndex(item => item.path === unref(route.path))
        ...
        closeTab(route: BackMenuModel) {
            const index = toRaw(this.tabsView).findIndex(item => item.path === unref(route.path))
            if (index !== -1) {
...
export function findMenu(tabsViewRoutes: BackMenuModel[], path: string): BackMenuModel | undefined {
